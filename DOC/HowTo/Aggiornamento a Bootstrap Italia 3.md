...
- Replicare lo stesso porting (hero+breadcrumb, chip/filtri, card→it-card) sulle altre famiglie di pagina (pubblicazioni, brevetti, impatti, risorse tecniche, luoghi, archivio news/eventi), seguendo l'ordine e i pattern già chiusi in `bs-playground`. Il banner v2 "semplice" (`bg-banner-progetti`/`section-muted p-3 primary-bg-c1`, già sostituito per Persone/Progetti/Ricerca) è duplicato anche in `template-parts/hero/luoghi.php`, `pubblicazioni.php`, `notizie.php`, `risorse-tecniche.php`, `brevetti.php`, `eventi.php`, `archive.php`, `spinoff.php`, `blog.php`, `mappasito.php` — stesso intervento da ripetere per ciascuno quando si arriva a quella famiglia.
