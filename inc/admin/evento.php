...
function dsi_register_evento_post_type() {
	...
	$args = array(
		...
		'has_archive'         => true,
				...
				'map_meta_cap'    => true,
				'description'     => __( "Il calendario scolastico", 'design_laboratori_italia' ),
		);
	register_post_type( 'evento', $args );

	$labels = array(
		'name'          => _x( 'Tipologia Evento', 'taxonomy general name', 'design_laboratori_italia' ),
		'singular_name' => _x( 'Tipologia Evento', 'taxonomy singular name', 'design_laboratori_italia' ),
...
function dsi_add_eventi_metaboxes() {
	...
	$cmb_sottotitolo->add_field(
		array(
			'id' => $prefix . 'descrizione',
			'name'        => __( 'Descrizione *', 'design_laboratori_italia' ),
			'desc' => __( 'Indicare una sintetica descrizione del Evento. Vincoli: 160 caratteri spazi inclusi.' , 'design_laboratori_italia' ),
			'type' => 'textarea',
			'attributes'    => array(
			'maxlength'  => '160',
			'required' => 'required'
	...
	$cmb_sottotitolo->add_field( 
		array(
			'id' => $prefix . '_promuovi_hero',
			'name' => __('Promuovi in hero', 'design_laboratori_italia' ),
			'type' => 'checkbox',
			'default' => false,
	...
	$cmb_sottotitolo->add_field( array(
		...
		'desc' => __( 'Selezione il <a href="edit.php?post_type=luogo">luogo del Laboratorio</a> in cui viene organizzato l\'evento. ' , 'design_laboratori_italia' ),
				...
				'attributes'    => array(
						'placeholder' =>  __( 'Seleziona il luogo', 'design_laboratori_italia' ),
	...
	$cmb_sottotitolo->add_field( array(
		...
		'attributes' => array(
			...
			'search'              => __( 'Digita l\'indirizzo del Luogo' , 'design_laboratori_italia' ),
	...
	$cmb_undercontent = new_cmb2_box( array(
		...
		'title'         => __( 'Dettagli Evento', 'design_laboratori_italia' ),
		'object_types' => array( 'evento' ),
		'context'      => 'normal',
		'priority'     => 'high',
	) );
		...
		$group_field_id = $cmb_undercontent->add_field( array(
				'id'          => $prefix . 'date',
				'name'        => __('<h1>Date</h1>' , 'design_laboratori_italia' ),
				'type'        => 'group',
				'description' => __( 'Se l\'evento si svolge in più giorni o fasi indica qui di seguito i diversi appuntamenti. Es: inizo attività, pausa pranzo, seconda sessione, etc', 'design_laboratori_italia' ),
				'options'     => array(
						'group_title'    => __( 'Fase {#}', 'design_laboratori_italia' ), // {#} gets replaced by row number
						'add_button'     => __( 'Aggiungi una data evento', 'design_laboratori_italia' ),
		...
		$cmb_undercontent->add_group_field( $group_field_id,  array(
				'id'      => 'data',
				'after'    => __( '<br>Data / orario ', 'design_laboratori_italia' ),
		...
		$cmb_undercontent->add_field( array(
				...
				'description' => __( 'Link alla circolare o alla notizia' , 'design_laboratori_italia' ),
				'type'    => 'pw_multiselect',
				'options' =>  dsi_get_approfondimenti_options(),
				'attributes' => array(
						'placeholder' =>  __( 'Seleziona articoli o circolari di approfondimento', 'design_laboratori_italia' ),
				),
		) );
	...
	$cmb_undercontent->add_field( array(
		...
		'name'    => __( 'Destinatari evento *', 'design_laboratori_italia' ),
		'desc'    => __( 'Comunità scolastica (rivolta a tutti) / solo personale scolastico (rivolta al personale della scuola)', 'design_laboratori_italia' ),
		'type'             => 'select',
		'show_option_none' => false,
		'options'          => array(
			'tutti' => __( 'Comunità scolastica', 'design_laboratori_italia' ),
			'personale'   => __( 'Personale Scolastico', 'design_laboratori_italia' )
		),
		'attributes' => array(
			'required' => 'required'
		),
	...
	$cmb_undercontent->add_field( array(
		...
		'name'        => __( 'Descrizione dei destinatari *', 'design_laboratori_italia' ),
		'desc' => __( 'Descrizione testuale dei principali interlocutori dell\'Evento. Questo elenco compare nel frontend..' , 'design_laboratori_italia' ),
		'type' => 'textarea',
		'attributes'    => array(
			'required' => 'required'
		),
	...
	$cmb_undercontent->add_field( array(
			...
			'name'       => __('Persone dell\'amministrazione ', 'design_laboratori_italia' ),
			'desc' => __( 'Link a schede del personale scolastico che parteciperanno all\'evento e che si vogliono mettere in risalto, per esempio un dirigente scolastico o un prof. che hanno promosso l\'evento ', 'design_laboratori_italia' ),
			'type'    => 'pw_multiselect',
	...
	$cmb_undercontent->add_field( array(
		...
		'name'    => __( 'Tipo evento *', 'design_laboratori_italia' ),
		'desc'    => __( 'Gratuito/ a pagamento', 'design_laboratori_italia' ),
		'type'             => 'select',
		'show_option_none' => false,
		'options'          => array(
			'gratis' => __( 'Gratuito', 'design_laboratori_italia' ),
			'pagamento'   => __( 'A Pagamento', 'design_laboratori_italia' )
		),
		'attributes' => array(
			'required' => 'required'
		),
	...
	$group_field_id = $cmb_undercontent->add_field( array(
		'id'          => $prefix . 'prezzo',
		'name'        => __('<h1>Prezzo</h1>' , 'design_laboratori_italia' ),
		'type'        => 'group',
		'description' => __( 'Biglietto intero - prezzo - testo che spiega le condizioni / Biglietto ridotto - prezzo - testo che spiega le condizioni / Biglietto gratuito - gratis - testo che spiega le condizioni', 'design_laboratori_italia' ),
		'options'     => array(
			'group_title'    => __( 'Prezzo {#}', 'design_laboratori_italia' ), // {#} gets replaced by row number
			'add_button'     => __( 'Aggiungi una fascia di prezzo', 'design_laboratori_italia' ),
			'remove_button'  => __( 'Rimuovi', 'design_laboratori_italia' ),
			'sortable'       => true,
			'closed'      => false, // true to have the groups closed by default
			//'remove_confirm' => esc_html__( 'Are you sure you want to remove?', 'cmb2' ), // Performs confirmation before removing group.
	...
	$cmb_undercontent->add_group_field( $group_field_id,  array(
		'id'      => 'tipo_biglietto',
		'name'    => __( 'Nome della tipologia di biglietto (Intero, Ridotto)', 'design_laboratori_italia' ),
		'type'             => 'text',
	) );

	$cmb_undercontent->add_group_field( $group_field_id,  array(
		'id'      => 'prezzo',
		'name'    => __( 'Prezzo', 'design_laboratori_italia' ),
		'type'             => 'text_money',
		'before_field' => '&euro;',
	) );
	$cmb_undercontent->add_group_field( $group_field_id,  array(
		'id'      => 'descrizione',
		'name'    => __( 'Descrizione della tipologia di biglietto.<br>Se si desidera inserire un video di YouTube è necessaria l\'opzione "Enable privacy-enhanced mode" che permette di pubblicare il video in modalità youtube-nocookie. ', 'design_laboratori_italia' ),
		'type'    => 'wysiwyg',
		'options' => array(
	...
	$cmb_undercontent->add_field( array(
		...
		'name'    => __( 'Organizzato dalla scuola', 'design_laboratori_italia' ),
		'type'             => 'select',
		'show_option_none' => false,
		'options'          => array(
	...
	$cmb_undercontent->add_field( array(
			...
			'name'       => __('Contatto: persona ', 'design_laboratori_italia' ),
			'desc' => __( 'Nome e cognome delle persone da contattare per informazioni sull\'Evento', 'design_laboratori_italia' ),
			'type'    => 'text',
						'attributes'    => array(
								'data-conditional-id'     => $prefix.'organizzato_da_scuola',
	...
	$cmb_undercontent->add_field( array(
			...
			'name'       => __('Contatto: telefono ', 'design_laboratori_italia' ),
			'desc' => __( 'Se non è un evento della scuola, inserire Numero di telefono per avere informazioni sull\'Evento', 'design_laboratori_italia' ),
			'type'    => 'text',
			'attributes'    => array(
				'data-conditional-id'     => $prefix.'organizzato_da_scuola',
				'data-conditional-value'  => "no",
	...
	$cmb_undercontent->add_field( array(
			...
			'name'       => __('Contatto: email ', 'design_laboratori_italia' ),
			'desc' => __( 'se non è un evento della scuola, Indirizzo email per avere informazioni sull\'Evento', 'design_laboratori_italia' ),
			'type'    => 'text_email',
			'attributes'    => array(
				'data-conditional-id'     => $prefix.'organizzato_da_scuola',
				'data-conditional-value'  => "no",
	...
	$cmb_undercontent->add_field( array(
			'id' => $prefix . 'website',
			'name'       => __('Sito web ', 'design_laboratori_italia' ),
			'desc' => __( 'Inserire il sito web dedicato all\'evento o della società che organizza l\'evento', 'design_laboratori_italia' ),
			'type'    => 'text_url'
		)
	);
	...
	$cmb_undercontent->add_field( array(
			...
			'name'       => __('Patrocinato da ', 'design_laboratori_italia' ),
			'desc' => __( 'Soggetto che patrocinia l\'evento', 'design_laboratori_italia' ),
			'type'    => 'text'
		)
	);
	$cmb_undercontent->add_field( array(
			...
			'desc' => __( 'Sponsor dell\'evento', 'design_laboratori_italia' ),
			'type'    => 'text'
		)
	);
	...
	$cmb_undercontent->add_field(
		array(
			'id' => $prefix . 'link_schede_relazioni',
			'name'    => __( 'Relazioni', 'design_laboratori_italia' ),
			'desc' => __( 'Inserisci qui tutti i documenti che ritieni rilevanti per l\'evento. Se devi caricare il documento <a href="post-new.php?post_type=documento">puoi creare una breve scheda di presentazione</a> (soluzione consigliata e più efficace per gli utenti del sito) oppure caricarlo direttamente nei campi che seguono. ' , 'design_laboratori_italia' ),
			'type'    => 'label',
			'column'  => true, // Output in the admin post-listing as a custom column. https://github.com/CMB2/CMB2/wiki/Field-Parameters#column
		)
	...
	$cmb_undercontent->add_field(
		array(
			'id'      => $prefix . 'progetti_correlati',
			'name'    => __( 'Progetti correlati', 'design_laboratori_italia' ),
			'desc'    => __( 'Seleziona la struttura responsabile del servizio ' , 'design_laboratori_italia' ),
			'type'    => 'pw_multiselect',
			'options' => dsi_get_progetti_options(),
		)
	);

	$cmb_undercontent->add_field(
		array(
			'id'      => $prefix . 'indirizzi-di-ricerca',
			'name'    => __( 'Indirizzi di ricerca', 'design_laboratori_italia' ),
			'desc'    => __( 'Seleziona la struttura responsabile del servizio ' , 'design_laboratori_italia' ),
			'type'    => 'pw_multiselect',
			'options' => dsi_get_indirizziricerca_options(),
		)
	);

	$cmb_side = new_cmb2_box( array(
		'id'           => $prefix . 'box_side',
		'title'        => __( 'Data inizio e fine evento *', 'design_laboratori_italia' ),
		'object_types' => array( 'evento' ),
		'context'      => 'side',
		'priority'     => 'high',
	) );
...
function sdi_evento_add_content_after_title($post) {
	if($post->post_type == "evento")
		_e('<span><i>il <b>Titolo</b> è il <b>Nome dell\'Evento</b></span><br><br>', 'design_laboratori_italia' );
}
...
function sdi_evento_add_content_before_editor($post) {
	if($post->post_type == "evento")
		_e('<h1>Descrizione Estesa dell\'evento</h1>', 'design_laboratori_italia' );
}
