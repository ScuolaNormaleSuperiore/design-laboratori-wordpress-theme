							...
							<ul class="link-list">
								<?php
									foreach ( $pages as $pg ) {
										?>
										<li>
											<a class="list-item large medium right-icon <?php if ( $post->ID === $pg->ID || $pg->ID === $post->post_parent ) echo "active"; ?>" href="<?php echo esc_url( get_permalink( $pg->ID ) ); ?>">
											...
											if ( $post->post_parent !== 0 && ($post->ID === $pg->ID || in_array( $post, $subspg ) ) ) {
												...
												<ul class="link-sublist">
													...
													foreach( $subspg as $subpg ) {
														?>
														<li>
															<a class="list-item <?php if ( $post->ID === $subpg->ID ) echo "active"; ?>" href="<?php echo esc_url( get_permalink( $subpg->ID ) ); ?>">
